...
func TestFutureMapRoundTrip(t *testing.T) {
	_, url := testServer(t, Options{})
	conn := dial(t, url)
	write(t, conn, room.Message{V: room.Protocol, Type: "join", UID: room.NewID(), Room: "test", Callsign: "A", Role: "gunner", Weapon: "mortar", Map: "future-map"})
