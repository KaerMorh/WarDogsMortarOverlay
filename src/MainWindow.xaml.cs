...
public partial class MainWindow:Window
{
    ...
    public async Task VerifyAndCapture()
    {
        ...
        try
        {
            ...
            Check(FindVisuals<CheckBox>(c.PzhCalibrationWindow!).Any(x=>x.Content as string=="开启校准补偿")&&FindVisuals<ComboBox>(c.PzhCalibrationWindow!).Any(x=>x.Items.Cast<object>().Any(i=>i as string=="XY 线性补偿"))&&Buttons(c.PzhCalibrationWindow!).Any(b=>b.Content as string=="从剪贴板添加")&&Buttons(c.PzhCalibrationWindow!).Any(b=>b.Content as string=="手动添加")&&Buttons(c.PzhCalibrationWindow!).Any(b=>b.Content as string=="重置全部"),"PZH window exposes mode, enable, smart/manual add and reset controls");
            Check(FindVisuals<ListBox>(c.PzhCalibrationWindow!).Any(x=>x.Background is SolidColorBrush {Color.R:17,Color.G:25,Color.B:37}),"PZH calibration list uses a dark readable background");c.PzhCalibrationWindow!.Close();
            c.State.SetOrigin(new(97.06,109.11),"验证");c.Hud.UpdateLayout();Check(c.PzhCalibrationMatchesCurrent&&c.Pref.Pzh.Shots.Count==0&&c.Pref.Pzh.Tilt==null&&!c.Pref.Pzh.Enabled&&Buttons(c.Hud).Any(b=>b.Content as string=="尚未校准"),"origin change automatically clears PZH calibration");
