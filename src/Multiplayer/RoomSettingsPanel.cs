...
public sealed class RoomSettingsPanel : StackPanel
{
    public RoomSettingsPanel(Controller c)
    {
        ...
        void Commit()
        {
            pref.ServerUrl=address.Text.Trim();
            pref.Room=room.Text.Trim().ToLowerInvariant();
            pref.Callsign=name.Text.Trim();
            Changed(); c.Save();
        }
        ...
        address.TextChanged += (_, _) => {if(!refreshing){pref.ServerUrl=address.Text.Trim();Changed();}};
        room.TextChanged += (_, _) => {if(!refreshing){pref.Room=room.Text.Trim().ToLowerInvariant();Changed();}};
        name.TextChanged += (_, _) => {if(!refreshing){pref.Callsign=name.Text.Trim();Changed();}};
        ...
        join.Click += async (_, _) => { Commit(); await c.Rooms.JoinAsync(); };
        ...
        var sync = new Button { Content = "同步房间状态", Padding = new Thickness(9, 6, 9, 6), Margin = new Thickness(0, 6, 0, 0) };
        sync.Click += async (_, _) => await c.Rooms.RequestSyncAsync(); Children.Add(sync);
        ...
        void Render()
        {
            ...
            try
            {
                ...
                join.Content=c.Rooms.JoinButtonText;join.IsEnabled=c.Rooms.CanJoin;
                leave.IsEnabled=c.Rooms.CanLeave;sync.IsEnabled=c.Rooms.CanSync;
        ...
        Loaded += (_, _) => { c.Updated += Render; Render(); }; Unloaded += (_, _) => { Commit(); c.Updated -= Render; };
